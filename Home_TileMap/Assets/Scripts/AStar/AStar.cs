...
public class AStar 
{
    ...
    public static List<Vector2Int> PathFind(GridMap gridMap, Vector2Int start, Vector2Int goal)
    {
        ...
        if (gridMap.IsValidPosition(start) && gridMap.IsValidPosition(goal))
        {
            ...
            Node current = gridMap.GetNode(start);  // current에 시작 위치의 노드를 찾기
            current.G = 0;
            current.H = GetHeuristic(current, goal);
            open.Add(current);                      // 오픈 리스트에 추가

            // A* 핵심 루틴 돌리기 시작
            while(open.Count > 0)       // 오픈 리스트가 비워질 때까지 계속 진행. (오픈 리스트가 비워지면 경로를 못 찾은 것)
            {
                open.Sort();            // F값을 기준으로 정렬
                current = open[0];      // F값이 가장 작은 노드를 current로 설정
                open.RemoveAt(0);       // 오픈 리스트에서 current 꺼내기

                if (current != goal)    // 꺼낸 current가 도착지점인지 확인
                {
                    // 도착지점이 아니면 계속 알고리즘 진행

                    close.Add(current); // 꺼낸 current 클로즈 리스트에 넣기
                    ...
                    for (int y = -1; y < 2; y++)
                    {
                        for (int x = -1; x < 2; x++)
                        {
                            ...
                            if (node == null)       // 맵 밖일 경우
                                continue;
                            if (node == current)    // current는 확인할 필요가 없다. (자기 자신이라서)
                                continue;
                            ...
                            if (node.G > current.G + distance)
                            {
                                ...
                                if (node.parent == null)                // 처음 사용하는 노드다 == 오픈 리스트에 안들어가 있다.
                                {
                                    node.H = GetHeuristic(node, goal);  // 휴리스틱 값 계산
                                    open.Add(node);                     // open리스트에 추가
                                }

                                // 오픈리스트에 있든 없든 공통으로 처리되는 부분
