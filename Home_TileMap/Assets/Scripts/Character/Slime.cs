...
public class Slime : MonoBehaviour
{
    ...
    {
        onDisable?.Invoke();    // 비활성화 되었다고 알림(레디큐에 다시 돌려주라는 신호를 보내주는 것이 주 용도)
    }
    ...
    {
        mainMaterial.SetFloat("_Phase_Thickness", 0.1f);    // 페이즈 진행 시 나올 중산 선의 두께 지정(보이게 만들기)
        ...
        while (timeElipsed < phaseDuration)                 // 특정 시간이 목표시간에 도달할 때 까지 반복
        {
            timeElipsed += Time.deltaTime;                  // 특정 시간을 프레임별로 누적시키기

            mainMaterial.SetFloat("_Phase_Split", timeElipsed * phaseDurationNormalize);    // 측정 시간을 정규화 시켜 split 값으로 변경

            yield return null;      // 다음 프레임까지 기다리기
        }

        mainMaterial.SetFloat("_Phase_Split", 1.0f);        // split을 최대치로 변경
        mainMaterial.SetFloat("_Phase_Thickness", 0.0f);    // 페이즈 진행 시 나올 중간 선의 두께 지정 (안보이게 만들기)
    ...
    IEnumerator StartDissolve()
    {
        ...
        float timeElipsed = 0.0f;                               // 측정 시간 초기화
        float dessolveDurationNormalize = 1 / dissolveDuration; // fade의 범위는 0~1이기 때문에 시간을 0~1사이의 범위로 정규화 시키기 위한 용도
    ...
    public void ShowOutline(bool isShow)
    {
        ...
        {
            mainMaterial.SetFloat("_OutLine_Thickness", 0.0f);              // 아웃라인의 두께 제거해서 안보이게 만들기
        }
    }
}
