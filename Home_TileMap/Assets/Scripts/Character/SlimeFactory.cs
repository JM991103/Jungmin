...
public class SlimeFactory : Singleton<SlimeFactory>
{
    ...
    protected override void Initialize()
    {
        ...
        for (int i = 0; i < poolSize; i++)          // poolSize만큼 반복
        {
            ...
            slime.onDisable += () =>
            {
                readyQueue.Enqueue(slime);  // 슬라임 게임 오브젝트가 disable될 때 레디큐로 되돌리기
            };
    ...
    public Slime GetSlime()
    {
        ...
        else
        {
            ...
            for (int i = poolSize; i < newSize; i++)    // 새풀의 빈곳에 새 슬라임 만들어서 추가(Insitialize와 거의 동일)
            {
                ...
                newpool[i] = slime;                     // 새풀의 뒤쪽에 추가한다는 것만 다름
                obj.SetActive(false);
            }
            pool = newpool;                             // 새풀을 풀로 설정
            poolSize = newSize;                         // 새크기로 풀크기 설정
