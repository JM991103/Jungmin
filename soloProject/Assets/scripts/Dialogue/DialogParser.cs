...
public class DialogParser : MonoBehaviour
{
    public Dialog[] parse(string _CSVFileName)
    {
        ...
        for (int i = 1; i < data.Length -1;)
        {
            ...
            dialog.name = row[1];                                       // 쪼개진 이름을 넣어줌(row[0] ID, row[1]이름, row[2]대사) 
            ...
            } while (row.Length > 1);                          // row[0] ID가 공백이면 반복하는 반복문

            dialog.contexts = contextList.ToArray();                    // dialog.contexts 배열에 contextList의 내용을 배열로 변환해 넣어준다

            dialogList.Add(dialog);                                     // dialogList에 이름과 대사를 넣어준다.
        }
